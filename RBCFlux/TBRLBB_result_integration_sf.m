...
for stack_idx = 1 : num_stack
    ...
    exp_data = cell(data_info.num_files, 1);
    exp_data_roi_list{stack_idx} = data_info.roi_list;
    % Load data
...
%             dataset_name, stack, roi_idx, record_idx));
        result_fp = fullfile(DataManager.fp_analysis_data_folder(dataset_name, stack), ...
            'vis_data_24', sprintf('%s_%s_ROI_%03d_%05d_vis_data.mat',...
            dataset_name, stack, roi_idx, record_idx));
        try
        ...
        linescanrate(counter) = data_info.parameters.linescan_rate_Hz;
        counter = counter + 1;
    end
    %% Processing single experiment data
    switch stack
        case 'TBRLBB20211129'
            roi_idx_pia = [1, 3, 6];
            roi_idx_ptr = [2, 4, 5, 7, 8];
        case 'TBRLBB20211119'
            roi_idx_pia = [1, 2, 3, 4, 5];
            roi_idx_ptr = [6, 7, 8, 9, 10, 11, 12, 13];
        case 'TBRLBB20211118'
            roi_idx_pia = [1, 2, 3, 4, 7];
            roi_idx_ptr = [5, 6, 8, 9, 10];
        case {'TBRLBB20211103', 'TBRLBB20211108'}
            roi_idx_pia = [1, 3, 4, 6, 7, 9];
            roi_idx_ptr = [2, 5, 8];
    end
%     switch stack
%         case 'JD230627M997_100723' 
%             roi_idx_pia = [7, 8, 9, 11, 13];
%             roi_idx_ptr = [10, 12];
%         case 'JD230627M997_100123' 
%             roi_idx_pia = [1, 4];
%             roi_idx_ptr = [2];    
%         case 'JD230627M002_100423' 
%             roi_idx_pia = [10];
%             roi_idx_ptr = [8, 9, 11]; 
%         case 'JD230627M002_92723' 
%             roi_idx_pia = [1, 2, 4, 5, 6];
%             roi_idx_ptr = [3, 7]; 
%         case 'JD230627M997_101123'
%             roi_idx_pia = [3, 15];
%             roi_idx_ptr = [16];       
%         case 'JD230627M945_101123' 
%             roi_idx_pia = [1, 3];
%             roi_idx_ptr = [2, 4, 5, 6];             
%     end
    ...
    all_exp_data{stack_idx,1} = exp_data;
    
    for iter_roi = 1 : num_roi 
        if ismember(roi_idx(iter_roi),[roi_idx_pia,roi_idx_ptr])
        
        tmp_roi_idx = roi_idx(iter_roi);
        ...
        tmp_data = cat(1, exp_data{tmp_list_idx}); %For cross correlation, we need to separate each 2 min trial, not concatenate all!
        %all_exp_data = exp_data{tmp_list_idx};
        ...
        vflux_n_edge = linspace(vflux_n_stat.prctile_val(2), vflux_n_stat.prctile_val(end-1), ...
            tmp_hdl.parameter.num_flux_edge);        
        %% Flux vs phase
        tmp_hdl.flux_vs_phase = tmp_hdl.analyze_vs_phase(phase_trace, ...
            flux_trace);        
        if vis_Q
            fig_hdl = tmp_hdl.vis_y_vs_phase_in_hist(phase_trace, flux_trace, ...
                phase_edge, flux_edge, tmp_hdl.flux_vs_phase, 'Radius phase (rad)', ...
                'Flux (RBC/s)', sprintf('ROI %d %s vessel', tmp_roi_idx, tmp_vs_type));
            tmp_hdl.save_figure(fig_hdl, {}, 'flux_vs_phase.png', ~dev_mode);
        end
        %% Radius vs phase
        tmp_hdl.radius_vs_phase = tmp_hdl.analyze_vs_phase(phase_trace, r_trace);
        if vis_Q
            fig_hdl = tmp_hdl.vis_y_vs_phase_in_hist(phase_trace, r_trace, ...
                phase_edge, r_edge, tmp_hdl.radius_vs_phase, 'Radius phase (rad)', ...
                'Radius (\mum)', sprintf('ROI %d %s vessel', tmp_roi_idx, tmp_vs_type));
            tmp_hdl.save_figure(fig_hdl, {}, 'radius_vs_phase.png', ~dev_mode);
        end
...
%         end
        ...
        %% Added 2/12/24
%         tmp_hdl.r_trace = r_trace;
%         tmp_hdl.flux_trace = flux_trace;
...
%% Integration
stack_data_tmp = stack_data; %47 total measurements, 5 days.
...
% For cross correlation calculation 2/12/24
all_exp_data = cat(1, all_exp_data{:});
all_exp_data = cat(1, all_exp_data{:});
%all_r = arrayfun(@(x) x.r_trace, stack_data, 'UniformOutput', false);
%all_r = cell2mat(all_r)
%all_flux = arrayfun(@(x) x.flux_trace, stack_data, 'UniformOutput', false);
%all_flux = cell2mat(all_flux);
%GET ALL RADIUS, FLUX TRACES INDIVIDUALLY, NO CONCATENATING FOR X CORR CALC
all_r = arrayfun(@(x) x.radius.r_or_mm.data, all_exp_data, 'UniformOutput',false);
all_flux = arrayfun(@(x) x.flux.per_frame_mm.data, all_exp_data, 'UniformOutput',false); %Take moving mean of this
%% Check data lengths
for i = 1:length(stack_data) %(after concatenation)
   tmp(i) = length(stack_data(i).flux.per_frame_mm.data);
end
% #########################################################################
%% David's figures
    %% Modulation depth - flux vs phase
    ...
    for i_p = 1 : num_fn
        ...
        a2c_list = arrayfun(@(x) x.flux_vs_phase.(tmp_fieldname).A2c, stack_data);
        a2c_fit_r2 = arrayfun(@(x) x.flux_vs_phase.(tmp_fieldname).R2, stack_data);

        fig_hdl = figure;
        ax_hdl = axes(fig_hdl);
%         sc_hdl = scatter(ax_hdl, a2c_list(is_pia_vessel_Q), a2c_fit_r2(is_pia_vessel_Q), ...
%             150, vessel_avg_r_list(is_pia_vessel_Q), 'o', 'filled');
        sc_hdl = scatter(ax_hdl, a2c_list(is_pia_vessel_Q), a2c_fit_r2(is_pia_vessel_Q), ...
            150, 2*vessel_avg_r_list(is_pia_vessel_Q), 'o', 'filled');
        hold(ax_hdl, 'on');
%         sc_hdl_1 = scatter(ax_hdl, a2c_list(~is_pia_vessel_Q), a2c_fit_r2(~is_pia_vessel_Q), ...
%             150, vessel_avg_r_list(~is_pia_vessel_Q), '*');
        sc_hdl_1 = scatter(ax_hdl, a2c_list(~is_pia_vessel_Q), a2c_fit_r2(~is_pia_vessel_Q), ...
        ...
        stat_str.pa_a2c_a = mean(a2c_list(~is_pia_vessel_Q));
        stat_str.pa_a2c_std = std(a2c_list(~is_pia_vessel_Q));    
        stat_str.pa_a2c_wa = sum(a2c_list(~is_pia_vessel_Q) .* a2c_fit_r2(~is_pia_vessel_Q)) / ...
            sum(a2c_fit_r2(~is_pia_vessel_Q));
        stat_str.pa_a2c2_wa = sum(a2c_list(~is_pia_vessel_Q).^2 .* a2c_fit_r2(~is_pia_vessel_Q)) / ...
            sum(a2c_fit_r2(~is_pia_vessel_Q));
        ...
        grid(ax_hdl, 'on');
        ax_hdl.XLabel.String = 'Flux phase modulation depth A/c';
        ...
        fig_fp = fullfile(int_fig_folder, sprintf('%s_%s_flux_phase_modulation_vsD%s_stat.png', ...
            dataset_name, int_stack_name, tmp_fieldname));
        fun_print_image_in_several_formats(fig_hdl, fig_fp);

        data_table = struct;
        data_table.a2c = a2c_list;
        data_table.a2c_fit_R2 = a2c_fit_r2;
    ...
    %% Modulation depth - radius vs phase 
    %Changing to diameter vs phase
    for i_p = 1 : num_fn
        tmp_fieldname = fit_fieldname{i_p};
        tmp_title = fit_title_name{i_p};

        a2c_list = arrayfun(@(x) x.radius_vs_phase.(tmp_fieldname).A2c, stack_data);
        a2c_fit_r2 = arrayfun(@(x) x.radius_vs_phase.(tmp_fieldname).R2, stack_data);
        ...
        grid(ax_hdl, 'on');
        ax_hdl.XLabel.String = 'Radius phase modulation depth A/c';
        ax_hdl.YLabel.String = 'Fitting R^2';
        ax_hdl.YLim = [0,1];
        sc_hdl.MarkerFaceAlpha = 0.6;
        sc_hdl_1.MarkerFaceAlpha = 0.6;
        ...
        fig_fp = fullfile(int_fig_folder, sprintf('%s_%s_radius_phase_modulation_vsD%s_stat.png', ...
            dataset_name, int_stack_name, tmp_fieldname));
        fun_print_image_in_several_formats(fig_hdl, fig_fp);
    ...
    for i_p = 1 : num_fn
        ...
        sc_hdl = scatter(ax_hdl, q2r_k_list(is_pia_vessel_Q), q2r_fit_r2_list(is_pia_vessel_Q), ...
            150, 2*vessel_avg_r_list(is_pia_vessel_Q), 'o', 'filled');
        hold(ax_hdl, 'on');
        sc_hdl_1 = scatter(ax_hdl, q2r_k_list(~is_pia_vessel_Q), q2r_fit_r2_list(~is_pia_vessel_Q), ...
            150, 2*vessel_avg_r_list(~is_pia_vessel_Q), '*');
        ...
        fig_fp = fullfile(int_fig_folder, sprintf('%s_%s_flux_vs_radius_fit_vsD%s_stat.png', ...
            dataset_name, int_stack_name, tmp_fieldname));
        fun_print_image_in_several_formats(fig_hdl, fig_fp);
...
%% Radius vs flux with intercept, plot slope
    for i_p = 1 : num_fn
        tmp_fieldname = fit_fieldname{i_p};
        tmp_title = fit_title_name{i_p};
        q2r_k_list = arrayfun(@(x) x.radius_vs_flux.normalized_intercept.(tmp_fieldname).k, stack_data);
        q2r_fit_r2_list = arrayfun(@(x) x.radius_vs_flux.normalized_intercept.(tmp_fieldname).R2, stack_data);
        q2r_k_SE_list = arrayfun(@(x) x.radius_vs_flux.normalized_intercept.(tmp_fieldname).k_SE, stack_data);
        ...
        int_fig_folder = '';
        fig_fp = fullfile(int_fig_folder, sprintf('%s_%s_flux_vs_radius_fit_slope_vsD_rest%s_stat.png', ...
            dataset_name, int_stack_name, tmp_fieldname));
        %fun_print_image_in_several_formats(fig_hdl, fig_fp);
...
%% Radius vs flux with intercept, plot intercept
    for i_p = 1 : num_fn
        tmp_fieldname = fit_fieldname{i_p};
        tmp_title = fit_title_name{i_p};
        q2r_b_list = arrayfun(@(x) x.radius_vs_flux.normalized_intercept.(tmp_fieldname).int, stack_data);
        q2r_fit_r2_list = arrayfun(@(x) x.radius_vs_flux.normalized_intercept.(tmp_fieldname).R2, stack_data);
        q2r_b_SE_list = arrayfun(@(x) x.radius_vs_flux.normalized_intercept.(tmp_fieldname).int_SE, stack_data);
        
        fig_hdl = figure;
        ax_hdl = axes(fig_hdl);
        sc_hdl = scatter(ax_hdl, q2r_b_list(is_pia_vessel_Q), q2r_fit_r2_list(is_pia_vessel_Q), ...
            150, 2*vessel_avg_r_list(is_pia_vessel_Q), 'o', 'filled');
        hold(ax_hdl, 'on');
        sc_hdl_1 = scatter(ax_hdl, q2r_b_list(~is_pia_vessel_Q), q2r_fit_r2_list(~is_pia_vessel_Q), ...
            150, 2*vessel_avg_r_list(~is_pia_vessel_Q), '*');

        stat_str = struct;
        stat_str.pia_a2c_a = mean(q2r_b_list(is_pia_vessel_Q));
        stat_str.pia_a2c_std = std(q2r_b_list(is_pia_vessel_Q));

        stat_str.pia_a2c_wa = sum(q2r_b_list(is_pia_vessel_Q) .* q2r_fit_r2_list(is_pia_vessel_Q)) / ...
            sum(q2r_fit_r2_list(is_pia_vessel_Q));
        stat_str.pia_a2c2_wa = sum(q2r_b_list(is_pia_vessel_Q).^2 .* q2r_fit_r2_list(is_pia_vessel_Q)) / ...
            sum(q2r_fit_r2_list(is_pia_vessel_Q));
        stat_str.pia_a2c_wstd = sqrt(stat_str.pia_a2c2_wa - stat_str.pia_a2c_wa^2);

        stat_str.pa_a2c_a = mean(q2r_b_list(~is_pia_vessel_Q));
        stat_str.pa_a2c_std = std(q2r_b_list(~is_pia_vessel_Q));    
        stat_str.pa_a2c_wa = sum(q2r_b_list(~is_pia_vessel_Q) .* q2r_fit_r2_list(~is_pia_vessel_Q)) / ...
            sum(q2r_fit_r2_list(~is_pia_vessel_Q));
        stat_str.pa_a2c2_wa = sum(q2r_b_list(~is_pia_vessel_Q).^2 .* q2r_fit_r2_list(~is_pia_vessel_Q)) / ...
            sum(q2r_fit_r2_list(~is_pia_vessel_Q));
        stat_str.pa_a2c_wstd = sqrt(stat_str.pa_a2c2_wa - stat_str.pa_a2c_wa^2);    
        ...
        grid(ax_hdl, 'on');
        ax_hdl.XLabel.String = 'Normalized flux to radius intercept';
        ax_hdl.YLabel.String = 'Fitting R^2';
        ax_hdl.YLim = [0,1];
        sc_hdl.MarkerFaceAlpha = 0.6;
        sc_hdl_1.MarkerFaceAlpha = 0.6;
        ...
        int_fig_folder = '';
        fig_fp = fullfile(int_fig_folder, sprintf('%s_%s_flux_vs_radius_fit_intercept_vsD_01HzStim%s_stat.png', ...
            dataset_name, int_stack_name, tmp_fieldname));
        fun_print_image_in_several_formats(fig_hdl, fig_fp);

        data_table = struct;
        data_table.q2r_b = q2r_b_list;
        data_table.q2r_fit_R2 = q2r_fit_r2_list;
...
%         ax_hdl.YLim = [0,1];
        ...
        int_fig_folder = '';
        fig_fp = fullfile(int_fig_folder, sprintf('%s_%s_flux_vs_radius_fit_slope_vsD_invVAR_rest%s_stat.png', ...
            dataset_name, int_stack_name, tmp_fieldname));
        fun_print_image_in_several_formats(fig_hdl, fig_fp);
    ...
    for i_p = 1 : num_fn
        ...
        sc_hdl = scatter(ax_hdl, q2Q_k_list(is_pia_vessel_Q), q2Q_fit_r2_list(is_pia_vessel_Q), ...
            150, vessel_avg_r_list(is_pia_vessel_Q), 'o', 'filled');
        hold(ax_hdl, 'on');
        sc_hdl_1 = scatter(ax_hdl, q2Q_k_list(~is_pia_vessel_Q), q2Q_fit_r2_list(~is_pia_vessel_Q), ...
            150, vessel_avg_r_list(~is_pia_vessel_Q), '*');
...
%         ax_hdl.YLim = [1e-4, 1];
        ...
        pen_data = tmp_fpsd_r(~is_pia_vessel_Q);
...
%         hold(ax_hdl, 'on');
%         sc_hdl_1 = scatter(ax_hdl, tmp_fpsd_r(~is_pia_vessel_Q & tmp_is_validQ), tmp_fpsd_flux(~is_pia_vessel_Q & tmp_is_validQ), ...
%             150, vessel_avg_r_list(~is_pia_vessel_Q & tmp_is_validQ), '*');
%         
%         stat_str = struct;
...
%         
%         stat_str.pa_ff_a = mean(tmp_fpsd_flux(~is_pia_vessel_Q & tmp_is_validQ));
%         stat_str.pa_ff_std = std(tmp_fpsd_flux(~is_pia_vessel_Q & tmp_is_validQ));
%         stat_str.pa_ff_a = mean(tmp_fpsd_flux(~is_pia_vessel_Q & tmp_is_validQ));
%         stat_str.pa_ff_std = std(tmp_fpsd_flux(~is_pia_vessel_Q & tmp_is_validQ));
%         
...
%         ax_hdl.Title.String = sprintf('Fractional power at > 5 Hz');
    ...
    sc_hdl = scatter(ax_hdl, vessel_avg_r_list(is_pia_vessel_Q & tmp_is_validQ), tmp_dr2r(is_pia_vessel_Q & tmp_is_validQ), ...
        150, 'o', 'filled');
    hold(ax_hdl, 'on');
    sc_hdl_1 = scatter(ax_hdl, vessel_avg_r_list(~is_pia_vessel_Q & tmp_is_validQ), tmp_dr2r(~is_pia_vessel_Q & tmp_is_validQ), ...
        150, '*');
    ...
    pen_data = tmp_dr2r(~is_pia_vessel_Q & tmp_is_validQ);
   ...
   %% Statistical Test between slopes
   q2r_b_SE_list = arrayfun(@(x) x.radius_vs_flux.normalized_intercept.(tmp_fieldname).k_SE, stack_data);
   pia_q2r_k_SE_list = q2r_k_SE_list(is_pia_vessel_Q);
   pa_q2r_k_SE_list = q2r_k_SE_list(~is_pia_vessel_Q);
   %Do weighted average and save for comparison with other experiments.
   
   
   
   
   
    %% Calculate cross correlation between each trial's flux and diameter traces.
    %Plot histogram of all lags, test if average lag is different from 0.
    %Use all_r and all_flux from above. Iterate over cells.
    frame_rate = 50.0633; %This is constant, after re-making frames
    clearvars all_lags all_corr
    %Keep track of which 2 minute trials come from which of the 47 pts -
    %calculate mean lag & std lag for each of the 47 pts.
    starting_trials = [1,36,101,149,157];
    for i = 1:length(exp_data_roi_list) %Group trials by roi when processing.
        current_start_trial = starting_trials(i);
        if i~=length(exp_data_roi_list)
            current_end_trial = starting_trials(i+1)-1;
        else
           current_end_trial = 171; 
        end
        roi_list_tmp = exp_data_roi_list{1,i};
        roi_unique_list = unique(roi_list_tmp);
        num_rois = numel(roi_unique_list);
        for j = 1:num_rois
           current_roi = roi_unique_list(j); 
           %Get trials to analyze together.
           trace_inds_tmp = ismember(roi_list_tmp,current_roi);
           all_r_stack = all_r(current_start_trial:1:current_end_trial);
           all_flux_stack = all_flux(current_start_trial:1:current_end_trial);
           r_tmp = all_r_stack(trace_inds_tmp);
           flux_tmp = all_flux_stack(trace_inds_tmp);
           for k = 1:length(r_tmp)
               r_trace = r_tmp{k,1};
               flux_trace = flux_tmp{k,1};
               r_trace_mean = r_trace - mean(r_trace);
               flux_trace_mean = flux_trace - mean(flux_trace);
               %Perform cross-correlation and save.
               [r,lags] = xcorr(2*r_trace_mean,flux_trace_mean,'normalized');
               
               %Find lag at max correlation
               [Corr_vec(k),LagInd(k)] = max(r);
               lag_vec(k) = lags(LagInd(k));
           end
           lag_vec = lag_vec/frame_rate;
           mean_lag(j) = mean(lag_vec);
           median_lag(j) = median(lag_vec);
           std_lag(j) = std(lag_vec);
           mean_corr(j) = mean(Corr_vec);
           median_corr(j) = median(Corr_vec);
           std_corr(j) = std(Corr_vec);
           clearvars lag_vec LagInd Corr_vec
        end
        all_lags{i} = median_lag;
        all_corr{i} = median_corr;
%         all_lags{i} = mean_lag;
%         all_corr{i} = mean_corr;
        all_lag_std{i} = std_lag;
        clearvars mean_lag mean_corr median_lag median_corr std_lag std_corr
    end
    all_lags = cell2mat(all_lags);
    all_corr = cell2mat(all_corr);
    all_std = cell2mat(all_lag_std);
    %% Plot
    figure
    histogram(all_corr,'BinWidth',0.1,'FaceAlpha',1)
    xlim([0 1])
    xlabel('Peak Correlation','Interpreter','latex');
    ylabel('Vessels','Interpreter','latex');
    title('Correlation, 47 Pial and Penetrating vessels','Interpreter','latex');
    ylim([0 16]);
    cd('')
    print(gcf,'-depsc','-painters','RestData_CrossCorrMedianCorrHist_47Vessels');
    
    corr_cutoff = 0.6;
    todel = all_corr < corr_cutoff;
    is_pia_restricted = is_pia_vessel_Q(~todel);
    ves_size_restricted = vessel_avg_r_list(~todel);
    
    figure
    histogram(all_lags(~todel),'BinWidth',0.1,'FaceAlpha',1);
    
    [h,p,ci,stats] = ttest(all_lags(~todel),0,'Tail','left');
    cstats.p = p;
    cstats.ttest_stats = stats;
    xline(mean(all_lags(~todel)),'r')
    xline(0);
    xlim([-1 0.5]);
    ylim([0 16]);
    
    xlabel('Lag at max. correlation (s)','Interpreter','latex');
    ylabel('Count (trials)','Interpreter','latex');
    str1 = sprintf('Mean = %.3f s +/- %.3f s',mean(all_lags(~todel)),std(all_lags(~todel)));
    str2 = sprintf('2 side t-test p = %.2e, $<$lag$>$ $\\neq$ 0',cstats.p); 
    title({'Diameter - flux cross correlation lag, 39 vessels',str1,str2},'Interpreter','latex');
    ax = gca; ax.TickLabelInterpreter = 'latex';
    print(gcf,'-depsc','-painters',['RestData_CrossCorrMedianLagHist_39Vessels_',strrep(num2str(corr_cutoff),'.','_')]);
    
    %Plot histograms sep by pia/pen
    figure
    all_lags_del = all_lags(~todel);
    histogram(all_lags_del(is_pia_restricted),'BinWidth',0.1,'FaceAlpha',0.5,'FaceColor','b')
    hold on
    histogram(all_lags_del(~is_pia_restricted),'BinWidth',0.1,'FaceAlpha',0.5,'FaceColor','r')
  
    figure;
    errorbar(2*ves_size_restricted,all_lags_del,all_std(~todel),'.')
    yline(0)
    xlabel('Average vessel diameter')
    ylabel('Flux-diameter lag (s)')
    %%
    
    
    r = {size(all_r,1)};
    lags = {size(all_r,1)};
    MaxC = zeros(size(all_r,1),1);
    LagInd = zeros(size(all_r,1),1);
    lag_vec = zeros(size(all_r,1),1);
    wind_lag_mean = zeros(size(all_r,1),1);
    wind_lag_std = zeros(size(all_r,1),1);
    wind_lag_wtdmean = zeros(size(all_r,1),1);
    wind_lag_wtdstd = zeros(size(all_r,1),1);
    wind_lag_median = zeros(size(all_r,1),1);
    wind_corr_mean = zeros(size(all_r,1),1);
    wind_corr_median = zeros(size(all_r,1),1);
    wind_corr_std = zeros(size(all_r,1),1);
    f_str = struct();
    phi_str = struct();
    
    params.Fs = frame_rate; %Interpolated rate is twice actual single depth rate
    params.pad = 2;
    params.fpass = [0 params.Fs/2]; %Hz, default is [0 Fs/2]
    params.err   = [2,0.05];
    params.trialave = 0;
    
    numpts = zeros(size(all_r,1),1);
%     wind_s = 20;
%     windpts = round(wind_s*frame_rate);
    
    for i = 1:size(all_r,1)
        tmp_r = all_r{i,1};
        tmp_flux = all_flux{i,1};
        %Mean-subtract
        tmp_r = tmp_r - mean(tmp_r);
        tmp_flux = tmp_flux - mean(tmp_flux);
        %Perform cross correlation.
        [r{i},lags{i}] = xcorr(2*tmp_r,tmp_flux,'normalized');
        
        %Find lag at max correlation
        [MaxC(i),LagInd(i)] = max(r{i});
        lag_vec(i) = lags{i}(LagInd(i));
        
        % Perform cross correlation on moving windows. %%%%%%%%%%%%
        tic;
        [corrtmp,lag_times] = fun_movingwindow_cross_corr(tmp_flux,tmp_r,20,frame_rate);
        toc
        %Find max positions
        corr_max = zeros(size(corrtmp,1),size(corrtmp,2));
        corr_vals = zeros(size(corrtmp,2),1);
        for j = 1:size(corrtmp,2)
            tmp_corr = corrtmp(:,j);
            %     corr_max(:,j) = tmp_corr == 1;
            [corr_vals(j),maxind] = max(tmp_corr);
            corr_max(maxind,j) = 1;
        end
        max_loc = zeros(size(corrtmp,2),1);
        for j = 1:size(corrtmp,2)
            tmp_corr = corr_max(:,j);
            max_loc(j) = find(tmp_corr);
%             if max_loc(i) ~= 21 && max_loc(i) ~= 22 && max_loc(i) > 15 && max_loc(i) < 28 %CHange these if changing window etc.
%                 plot(time_vec(i),lags(max_loc(i)),'r.');
%                 %     counter = counter + 1
%             end
        end
        max_lag_times = lag_times(max_loc)';
        
        %Save statistics for later plotting.
        wind_lag_mean(i) = mean(max_lag_times);
        wind_lag_wtdmean(i) = sum(max_lag_times.*corr_vals)/sum(corr_vals);
        wind_lag_std(i) = std(max_lag_times);
        df_fac = sum(corr_vals)^2/((sum(corr_vals)^2) ...
            - sum(corr_vals.^2));
        wind_lag_wtdstd(i) = sqrt((sum(corr_vals.*max_lag_times.^2)/sum(corr_vals) - wind_lag_wtdmean(i)^2)*df_fac);
        wind_lag_median(i) = median(max_lag_times);
        wind_corr_mean(i) = mean(corr_vals);
        wind_corr_median(i) = median(corr_vals);
        wind_corr_std(i) = std(corr_vals);
        numpts(i) = numel(corr_vals);
        i
        %clearvars corrtmp lag_times maxind  tmp_corr
        
        % Perform coherence / phase calculations %%%%%%%%%%%%%

        
        T = length(tmp_r)/frame_rate;
        BW = 0.04;
        params.tapers = [round(T*BW),round(2*T*BW-1)]; %Time-BW product and number of tapers
        [C,phi,S12,S1,S2,f,confC,phistd,Cerr] = coherencyc(tmp_r,tmp_flux,params);
        f_str(i).f = f;
        f_str(i).phi = phi;
        f_str(i).C = C;
    end
    ...
    figure;
    for i = 1:171
       plot(f_str(i).f,f_str(i).phi,'Color',[0 0 0 0.1]) 
        hold on;
    end
    xlim([0 0.5])
    ylim([-pi pi]);
    ...
    figure;
    for i = 1:171
        plot(f_str(i).f,f_str(i).C,'Color',[0 0 0 0.1]) 
        hold on;
    end
    xlim([0 0.5])
    ylim([0 1]);
    ...
    %% Plot moving wind results
    figure
    %errorbar(wind_lag_mean,wind_corr_mean,wind_corr_std,wind_corr_std,wind_lag_std,wind_lag_std,"o")
    %errorbar(wind_lag_mean,wind_corr_mean,wind_lag_std,"horizontal",".","Color",[0,0,1,0.2])
    errorbar(wind_lag_mean,wind_corr_mean,wind_lag_std./sqrt(numpts),"horizontal",".","Color",[0,0,1,0.2])
    xline(0);
    xlim([-1.5 1.5]);
    xlabel('Lag (s)','Interpreter','latex');
    ylabel('Correlation','Interpreter','latex');
    title({'Moving Window Cross Correlations','All values are mean +/- SE'},'Interpreter','latex');
    
    figure
    histogram(wind_lag_mean,'BinWidth',0.1,'FaceColor','b');
    xlim([-1.5 1.5])
    xline(0);
    xlabel('Lag (s)','Interpreter','latex');
    ylabel('Trials','Interpreter','latex');
    [h,p,ci,stats] = ttest(wind_lag_mean,0,'Tail','left');
    str1 = sprintf('Mean = %.3f s +/- %.3f s',mean(wind_lag_mean),std(wind_lag_mean));
    str2 = sprintf('1 side t-test p = %.2e, $<$lag$>$ $\\neq$ 0',cstats.p); 
    title({'Diameter - flux moving window cross correlation lag',str1,str2},'Interpreter','latex');
    %% Plot total correlation results
    todel = MaxC < 0.7; %Use 0.4 |C| cutoff
    lag_vec(todel) = [];
    MaxC(todel) = [];
    LagInd(todel) = [];
    lag_vec = lag_vec/frame_rate;
    ...
    figure;
    histogram(lag_vec,'BinWidth',0.01,'FaceAlpha',1)
    xline(0,'k','LineWidth',2);
    ylim([0 35]);
    
    %Calc summary statistics
    cstats = struct();
    cstats.meanC = mean(lag_vec);
    cstats.stdC = std(lag_vec);
    cstats.medianC = median(lag_vec);
    cstats.wtdmeanC = sum(lag_vec.*MaxC)/sum(MaxC);
    n =sum(MaxC)^2 / sum(MaxC.^2);
    cstats.wtdstdC = sqrt((mean(lag_vec.^2) - mean(lag_vec)^2)*n/(n-1));
    %t-test lag ~= 0?
    [h,p,ci,stats] = ttest(lag_vec,0,'Tail','left');
    cstats.p = p;
    cstats.ttest_stats = stats;
    xline(cstats.meanC,'r')
    
    xlabel('Lag at max. correlation (s)','Interpreter','latex');
    ylabel('Count (trials)','Interpreter','latex');
    str1 = sprintf('Mean = %.3f s +/- %.3f s',cstats.meanC,cstats.stdC);
    str2 = sprintf('1 side t-test p = %.2e, $<$lag$>$ $\\neq$ 0',cstats.p); 
    title({'Diameter - flux cross correlation lag',str1,str2},'Interpreter','latex');
    ax = gca; ax.TickLabelInterpreter = 'latex';
    %%
    cd('')
    print(gcf,'-depsc','-painters','RestData_CrossCorrLagHist');
    
    figure;
    histogram(MaxC,'BinWidth',0.05,'FaceAlpha',1)
    xlabel('Maximum correlation','Interpreter','latex');
    ylabel('Count (trials)','Interpreter','latex');
    str1 = sprintf('Mean = %.3f s +/- %.3f s',mean(MaxC),std(MaxC));
    title({'Diameter - flux cross correlation max correlation',str1},'Interpreter','latex');
    ax = gca; ax.TickLabelInterpreter = 'latex';
    ylim([0 35]); xlim([0.4 1]);
    print(gcf,'-depsc','-painters','RestData_CrossCorrCorrHist');
    ...
    figure
    for i = 1:length(lags)
        plot(lags{i}/frame_rate,r{i},'Color',[0,0,0,0.1]);
        hold on;
    end
    xlim([-20 20])
    %Calculate Average and standard deviation of traces - need to
    %interpolate.
...
plt_group(:, end+1) = {'penetrating', ~is_pia_vessel_Q};
...
data_label = arrayfun(@(x) sprintf('%sROI%03d%05d', x.stack(7:end), x.roi_idx, x.record_idx),...
    ...
    legend(ax_hdl_1, sprintf('#Pia: %d', nnz(is_pia_vessel_Q)),...
        sprintf('#Penetrating: %d', nnz(~is_pia_vessel_Q)), ...
        'Location', 'northwest');
