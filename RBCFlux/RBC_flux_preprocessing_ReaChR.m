...
rui_folder_root = '';
rui_data_folder_list = {'20240129R152'}; %'20231219R155Ec'
%%
for raw_data_idx = 1
    ...
    if size(folder_labels,2) > 1
        stack = strjoin(folder_labels(1:2), '_');
    else
        stack = folder_labels{1};
    end
    data_info = struct;
    ...
    file_basename = cellfun(@str2num, file_basename,'UniformOutput',false);
    file_basename(:,2) = []; %If percent power is included
    [file_basename, s_idx] = sortrows(file_basename, 'ascend');
   
    if iscell(file_basename)
        data_info.roi_list = cell2mat(file_basename(:, 1));
        data_info.record_idx_list = cell2mat(file_basename(:, 2));
    else
    ...
    for iter_file = 1 : data_info.num_files
        ...
        im_rbc = im(:, :, 1:2:end); %All CFSE RBC images         
